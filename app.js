...
function renderPractice(entry) {
  ...
  document.getElementById("app").innerHTML = `
    ...
    <section class="chapter-content">
      ...
      <button class="btn btn-ghost btn-small" id="backToTheory">${esc(t("nav.backTheory"))}</button>
      ...
      ${finished ? `
        ...
        <div id="hintZone"></div>
        ${stepIdx > 0 ? `<div class="nav-row"><button class="btn btn-ghost btn-small" id="prevStep">${esc(t("nav.backStep"))}</button><span></span></div>` : ""}
      `}
    </section>`;
  bindSidebarNav();
  document.getElementById("backToTheory").addEventListener("click", () => {
    go({ type: "chapter", chapterId: chap.id, mode: "theory" });
  });
  const prevBtn = document.getElementById("prevStep");
  if (prevBtn) prevBtn.addEventListener("click", () => { rt.practiceStep = stepIdx - 1; render(); });
...
function renderExam(entry) {
  ...
  document.getElementById("app").innerHTML = `
    ...
    <section class="chapter-content">
      <p class="crumb">${esc(entry.moduleTitle[state.lang])} — ${esc(chap.title[state.lang])}</p>
      <button class="btn btn-ghost btn-small" id="backToPractice">${esc(t("nav.backPractice"))}</button>
